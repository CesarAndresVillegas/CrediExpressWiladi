...
.controller('RutaCtrl', function($scope, RutasServices, PagosServices, $ionicPopup, $state, $stateParams, $rootScope, $http, SharedService, $timeout) {
	...
	$scope.cargarRutas = function (){
		RutasServices.cargarRutas($rootScope.rutas_id, $scope,$rootScope, $http)
		.success(function(data) {
			$rootScope.hide();
		...
		.error(function(data) {
			...
			var alertPopup = $ionicPopup.alert({
				title: 'SISTEMA',
				template: 'No se han encontrado clientes pendientes.'
			});
		});
	};
